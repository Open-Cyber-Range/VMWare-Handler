...
type AlbAutoScaleLaunchConfig struct {
	...
	Mesos *AlbAutoScaleMesosSettings `json:"mesos,omitempty"`
	Openstack *AlbAutoScaleOpenStackSettings `json:"openstack,omitempty"`
	// If set to True, ServerAutoscalePolicy will use the autoscaling group (external_autoscaling_groups) from Pool to perform scale up and scale down. Pool should have single autoscaling group configured. Default value when not specified in API or module is interpreted by ALB Controller as true. 
